# Documentação — Nossa História

Índice da documentação do projeto.

| Documento | Descrição |
|-----------|-----------|
| [../README.md](../README.md) | Visão geral, conceito e resumo do app |
| [ARQUITETURA.md](./ARQUITETURA.md) | Arquitetura do sistema e fluxos |
| [BANCO-DE-DADOS.md](./BANCO-DE-DADOS.md) | Esquema completo do banco de dados |
| [FUNCIONALIDADES.md](./FUNCIONALIDADES.md) | Detalhamento de cada funcionalidade |
| [MVP-E-ROADMAP.md](./MVP-E-ROADMAP.md) | Escopo do MVP e roadmap |
| [DESIGN.md](./DESIGN.md) | Diretrizes de design e UI |
| [SOLID.md](./SOLID.md) | Princípios SOLID e organização do código |
| [../CONTRIBUTING.md](../CONTRIBUTING.md) | Guia de contribuição |
